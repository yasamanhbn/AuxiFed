...
def get_dataset(config, mode='train'):
    ...
    if config.DATASETTYPE == 'Mnist':
          dataset = MNIST(root=DATA_DIR, train=True if mode=='train' else False, download=True, transform=transform)
    else:
          dataset = datasets.EMNIST(root=DATA_DIR, split='letters', train=True if mode=='train' else False, download=True, transform=transform)
...
def generate_data(config):
    ...
    parser.add_argument("--min_sample", type=int, default=5500, help="Min number of samples per user.")
    ...
    parser.add_argument("--alpha", type=float, default=config.ALPHA, help="alpha in Dirichelt distribution (smaller means larger heterogeneity)")
    parser.add_argument("--n_user", type=int, default=config.NUM_CLIENTS, help="number of local clients, should be muitiple of 10.")
    ...
    path_prefix = f'{config.DATASETTYPE}-u{args.n_user}c{args.n_class}-alpha{args.alpha}-ratio{args.sampling_ratio}'
